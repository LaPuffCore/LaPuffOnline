...
function MiniMap({ lat, lng, address, city }) {
  // FIX 1: CARTO OSM PROXY (Literal data, No API keys, No Auth errors)
  const mapCenter = (lat && lng) ? `${lng},${lat}` : encodeURIComponent(`${address || ''} ${city || ''}`);
  const staticMapUrl = (lat && lng) 
    ? `https://basemaps.cartocdn.com/light_all/14/${lng}/${lat}.png` // Simple tile fallback
    : `https://static-maps.yandex.ru/1.x/?lang=en_US&ll=${mapCenter}&z=14&l=map&size=450,200`; // Address fallback if coords missing

  // Link directly to OSM search/coords
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0 overflow-hidden bg-[#e5e3df]">
        <img
          src={lat && lng ? `https://render.openstreetmap.org/cgi-bin/export?bbox=${lng-0.01},${lat-0.01},${lng+0.01},${lat+0.01}&scale=10000&format=png` : staticMapUrl}
          alt="Sector Location"
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t-2 border-black p-2 flex items-center justify-between gap-2 z-20">
        ...
        <div className="flex-shrink-0 bg-black text-white px-2 py-1 rounded-lg text-[9px] font-black group-hover:bg-[#7C3AED] transition-colors flex items-center gap-1">
          OSM <span className="text-[10px]">↗</span>
        </div>
      </div>
    </a>
  );
...
export default function EventDetailPopup({ event, onClose, onNext, onPrev }) {
  ...
  useEffect(() => {
    ...
    // Scroll lock for the main body only
  ...
  return (
    <>
      {/* FIX 2: CONTINUOUS VIEWPORT BLUR
          Fixed position covers the screen. Pointer-events-none ensures it 
          doesn't block clicks to the popup or background.
      */}
      <div 
        className="fixed inset-0 z-[9998] backdrop-blur-xl bg-white/10 pointer-events-none" 
      />

      {/* Main Overlay Wrapper - Absolute allows it to grow with content */}
      <div 
        className="absolute inset-0 z-[9999] flex items-start justify-center p-4 min-h-screen" 
        onClick={onClose}
      >
        <div
