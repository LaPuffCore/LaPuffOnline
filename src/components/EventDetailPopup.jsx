...
export default function EventDetailPopup({ event, onClose, onNext, onPrev }) {
  ...
  useEffect(() => {
    // FIXED: Capture scroll position to anchor the tile relative to current view
  ...
  return (
    <div className="fixed inset-0 z-[100000] overflow-y-auto custom-scrollbar">
      {/* Background Blur */}
      <div 
      ...
      {/* SHIFTED CONTENT: padding-top accounts for the user's relative scroll + header gap */}
      <div 
        className="relative min-h-screen flex flex-col items-center p-4"
        style={{ paddingTop: `${currentScrollY + 64}px` }}
      >
        
        {/* SIDE ARROWS: Flanking the tile regardless of tile view connection */}
        <div className="flex items-center justify-center gap-4 lg:gap-12 w-full max-w-5xl mb-12">
          ...
          <button 
            ...
            className={`w-14 h-14 lg:w-16 lg:h-16 flex items-center justify-center bg-white border-4 border-black rounded-full text-2xl lg:text-3xl shadow-[6px_6px_0px_black] transition-all
              ${onPrev ? 'hover:bg-black hover:text-white hover:-translate-x-1 active:translate-x-0' : 'opacity-20 grayscale cursor-not-allowed'}`}
          >
            ←
          </button>
          ...
          <button 
            ...
            className={`w-14 h-14 lg:w-16 lg:h-16 flex items-center justify-center bg-white border-4 border-black rounded-full text-2xl lg:text-3xl shadow-[6px_6px_0px_black] transition-all
              ${onNext ? 'hover:bg-black hover:text-white hover:translate-x-1 active:translate-x-0' : 'opacity-20 grayscale cursor-not-allowed'}`}
          >
            →
          </button>
        </div>
