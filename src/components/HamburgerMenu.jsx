...
export default function HamburgerMenu({ events }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-14 w-56 bg-white border-3 border-black rounded-3xl shadow-[8px_8px_0px_black] z-[999] overflow-hidden">
          <div className="px-5 pt-4 pb-3 border-b-2 border-black">
            <h3 className="font-black text-base">Menu</h3>
          </div>
          <div className="p-2">
            <Link to="/favorites" onClick={() => setOpen(false)}
              className="w-full text-left px-4 py-3 rounded-2xl hover:bg-violet-50 font-bold flex items-center gap-3 block transition-colors">
            ...
            <div className="border-t border-gray-100 mt-2 pt-2 px-4 pb-3">
              <p className="text-xs text-gray-400 font-medium">LaPuff Online · NYC Events</p>
            </div>
          </div>
        </div>
      )}
