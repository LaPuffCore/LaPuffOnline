...
export default function MapView({ events }) {
  ...
  useEffect(() => {
    ...
    const freezeZoom = 9.5; // constant range starts at 9.5 (inclusive)
    const mppFreeze = metersPerPixel(freezeZoom);
    // Anchor frozen px to the prior visual baseline then scale as before
    const ORIGINAL_PX_AT_FREEZE = 220;
    const FROZEN_PX_AT_FREEZE = Math.round(ORIGINAL_PX_AT_FREEZE * 0.7); // ~154px
    const SCALE_ABOVE_FREEZE = 1.44; // cumulative scale for zoom >= 9.5
    const desiredMeters = FROZEN_PX_AT_FREEZE * mppFreeze * SCALE_ABOVE_FREEZE; // apply cumulative scale to real-world reach
    ...
    const updateHeatRadius = () => {
      ...
      if (zoom >= freezeZoom) {
        ...
        // For zooms 9 -> freezeZoom (9.5): interpolate px linearly between px@9 and px@9.5
        const PX_AT_9 = 200;
      ...
      // - Tier4 (red): zoom 9 -> freezeZoom ramps down (weaker at wide zooms), then fixed in frozen range
