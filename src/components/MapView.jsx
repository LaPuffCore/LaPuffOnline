...
export default function MapView({ events }) {
  ...
  function applyReal3DLayers(map, isHeatmap) {
    ...
    try {
      // Single unified layer — no separate baseplate + extrusion competing for Z-space.
      // Height interpolation creates the zoom-based baseplate → full-3D transition natively:
      //   zoom 11–12.5: height=0  → flat footprint (baseplate phase, memory-efficient)
      //   zoom 12.5–13.5: lerps   → smooth transition
      //   zoom 13.5+: full height  → real 3D buildings
      //
      // minzoom decreased by 1 tick (11 vs 12) so buildings render/transition sooner.
      //
      // within filter strictly clips to NYC 5-borough bbox so NJ/CT buildings never render.
      // Default color is our red palette (no cyan default) so there is no flash on tile load.
      map.addLayer({
        id: 'real3d-buildings',
        type: 'fill-extrusion',
        source: 'openmaptiles',
