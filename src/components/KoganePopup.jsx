...
// How many px each scroll image overlaps into the green screen on each side
const SEAM = 60;
...
export default function KoganePopup({ onClose }) {
  ...
  // Lock body scroll + measure header height
  useEffect(() => {
  ...
  // Inject CSS keyframes once, clean up on unmount
  useEffect(() => {
    if (!document.getElementById('kogane-css')) {
      ...
      s.textContent = `
        ...
        @keyframes kogane-glow { 0%,100%{box-shadow:inset 0 0 80px rgba(0,255,60,0.22),inset 0 0 200px rgba(0,180,30,0.1),0 0 80px rgba(0,255,60,0.45)} 50%{box-shadow:inset 0 0 100px rgba(0,255,60,0.32),inset 0 0 240px rgba(0,200,40,0.16),0 0 120px rgba(0,255,60,0.6)} }
      `;
  ...
  // After 2 s, measure content height and trigger expand animation
  useEffect(() => {
    const t = setTimeout(() => {
      const inner = contentInnerRef.current;
      const h = inner ? Math.max(inner.scrollHeight, 200) : 800;
      setScreenH(h);
      setExpanded(true);
    }, 2000);
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Scroll container — ~2x scaled, within viewport */}
        <div style={{ width: 'min(1600px, 96vw)', position: 'relative' }}>
          ...
          <img
            src={topSrc}
            alt="scroll top"
            style={{ display: 'block', width: '100%', position: 'relative', zIndex: 3 }}
          />

          {/* ── GREEN SCREEN — expands like a scroll unrolling ── */}
          <div
            style={{
              marginTop: `-${SEAM}px`,
          ...
          >
            ...
            {/* Rules content — padded by SEAM so text clears the scroll image overlaps */}
            <div
              ref={contentInnerRef}
              style={{
                paddingTop: `${SEAM}px`,
            ...
            >
              <h2 style={{
                textAlign: 'center', fontWeight: 900, fontSize: '26px',
                marginBottom: '20px', color: '#0d1a00',
                textShadow: '0 1px 4px rgba(0,0,0,0.45), 0 0 14px rgba(0,60,0,0.4)',
                letterSpacing: '0.04em',
              }}>
                CLOUT CULLING GAME RULES
              </h2>

              <div style={{ fontSize: '15px', lineHeight: 1.85, color: '#071000' }}>
                ...
                <div style={{ marginBottom: '18px' }}>
                  <p style={{ ...ruleTextStyle, marginBottom: '10px' }}><strong>VII.</strong> Players can expend a set amount of points as determined by Game Master LaPuff to engage one of the three following options:</p>
                  <ol style={{ marginLeft: '28px', paddingLeft: '6px', fontWeight: 800, listStyleType: 'upper-alpha', textShadow: '0 1px 2px rgba(0,0,0,0.28)' }}>
                    <li style={{ marginBottom: '9px' }}>Players may add a rule to the Clout Culling Games provided that the rule described does not end the Games. Rules added may not be subtracted.</li>
                    <li style={{ marginBottom: '9px' }}>Players may add a site function to the site which hosts the Clout Culling Games - if this function adds a way for Players to gain or lose points it will be accordingly balanced by Games Master LaPuff.</li>
                    <li style={{ marginBottom: '9px' }}>Players may claim a zip region as theirs to form as an Official Clout Colony gaining a name of their choosing, color of their choosing, and other perks as to be determined by the development of the Game.</li>
                  </ol>
                </div>
                ...
                <p style={{ ...ruleTextStyle, marginBottom: 0 }}><strong>IX.</strong> If a Player's score remains the same for 28 days they will be subject to clout removal and they will enter 'Fallen Off' status.</p>
              </div>
            </div>
          </div>

          {/* ── KOGANE BOTTOM — overlaps green screen bottom by SEAM ── */}
          <img
            src={bottomSrc}
            alt="scroll bottom"
            style={{
