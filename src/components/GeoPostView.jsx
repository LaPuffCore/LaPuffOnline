  ...
  return (
    <div
      ...
      style={{
        ...
        height: '100%',
        overflow: 'visible',
        // Elevate entire tile on hover so corner buttons float above neighboring tiles
        zIndex: tileHovered ? 9999 : 1,
      ...
      {post.image_url && !isSplitTile && (
        ...
        >
          <img
            src={post.image_url}
            alt="post"
            className={`absolute inset-0 block w-full h-full object-cover select-none pointer-events-none transition-opacity duration-200 ${imgLoaded ? 'opacity-100' : 'opacity-0'}`}
...
export default function GeoPostView({ session, headerCollapsed = false }) {
  ...
  useEffect(() => {
    ...
    const computeTargetRow = () => {
      ...
      // Shape-aware row stride: round has 12px gap, square has gap=0 and
      // rows scaled by squareRowScale. Reading from inline style would be
      // racy, so compute deterministically from the same inputs the JSX uses.
      const halfRowPx = tileShape === 'square'
        ? Math.max(1, (desktopUnitHeight - 12) / 2 * squareRowScale)
        : Math.max(1, (desktopUnitHeight - 12) / 2 + 12);
  ...
  }, [desktopUnitHeight, visiblePosts.length, canShowMore, canShowLess, applyPanelRow, feedLayout, filterPanelMode, tileViewKey, tileShape, squareRowScale]);
  ...
  useEffect(() => {
    ...
    // Safety net: if mouse leaves the window while dragging, clean up so ghost doesn't get stuck
    const onMouseLeaveWindow = (e) => {
      if (e.relatedTarget === null && window._gpDragState?.active) {
        onMouseUp();
      }
    };
